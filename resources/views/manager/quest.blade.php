  ...
  <tbody>
    ...
    <tr id="Q{{$squest->id}}">
     ...
     {{$date->locale('fr_FR')->isoFormat('dddd DD MMMM')}}
      ...
      <td class="text-center">
      @if($squest->state == 0) 
      <a href="/manager/confirm/quest?Qt={{$squest->id}}#QT={{$squest->id}}" class="btn btn-success">Confirmer</a> <b>|</b> 
      <a href="/manager/abort/quest?Qt={{$squest->id}}#QT={{$squest->id}}" class="btn btn-danger">Avorter</a> 
