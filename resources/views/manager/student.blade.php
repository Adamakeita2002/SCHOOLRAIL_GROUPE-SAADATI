...
<!-- GENJUTSU --> <form ></form> <!-- GENJUTSU -->
  ...
  <div class="row">
    ...
    <form action="{{ URL::to('/manager/modifyStudent') }}" method="post" enctype="multipart/form-data" id="theform2">
      <!--EDIT FORM-->
      <input type="hidden" value="{{ csrf_token() }}" name="_token">
  ...
  $(function()
{
  ...
  <div class="col-sm-6"><!-- COL 6 -->
    ...
    <p class="text-center"><button class="btn btn-danger" data-toggle="modal" data-target="#MD1{{$Ustudent->id}}" ><b>SUPPRIMER</b></button></p>
      ...
      <form action="{{ URL::to('/manager/deleteStudent') }}" method="post" enctype="multipart/form-data" id="theform3">
        <!--DELETE FORM-->
        <input type="hidden" value="{{ csrf_token() }}" name="_token">
