...
@if (!empty($student))
<h2 class="text-center" ><b>{{$student->name}}</b> {{$student->surname}}</h2>
          <div class="accordion pt-2 pb-2"  id="accordionExample{{$student->classroom->id}}">
            ...
            <tbody>
              ...
              <tr> 
                ...
                <?php $V=$student->versements->where('type',1)->count();  ?>
                ...
                <td> <b>Aucun Versement</b> </td>
                  ...
                  <td>
                    <?php $i =1;  ?>
                    @foreach($student->versements->where('type',1) as $versement) 
                    <span class="badge badge-primary @if($i==1)badge-dark @endif" >
                       <?php echo $i;  ?> 
              ...
              <div class="row">
                  <div class="col-sm-6">
                    <form></form>
                    <form action="/manager/CreateScolarite" id="sco{{$student->id}}" method="post" enctype="multipart/form-data">
                      <input type="hidden" value="{{ csrf_token() }}" name="_token">
                      <input type="hidden" value="{{$student->id}}" name="student_id">
                      <input type="number" class="form-control" name="amount" placeholder="Montant" required="">
                      <button type="submit" class="btn btn-primary form-control  mt-1" ><b>Valider</b></button>
                    </form>
                  </div>  
                  <div class="col-sm-6">
                    <form></form>
                    <form action="/manager/CreateScolaritePrint" id="sco{{$student->id}}" method="post" enctype="multipart/form-data">
                      <input type="hidden" value="{{ csrf_token() }}" name="_token">
                      <input type="hidden" value="{{$student->id}}" name="student_id">
