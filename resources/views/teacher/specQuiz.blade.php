  ...
  <?php $quiz="activve" ;?>
      ...
      <!--END NAVBAR -->
        ...
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">{{$specSubject->name}} en {{$specSubject->classroom->name}} </li>
          </ol>
        </nav>
    ...
    <div class="col-md-8">
      ...
      <h1 class="my-4 card-header text-center bg-primary text-white">QUESTIONS / REPONSES <br>
        <b>({{$specSubject->name}})</b> <br>
        <b>({{$specSubject->classroom->name}})</b>
      </h1>

@if ($specSubject->questions)

        @if ($specSubject->questions->count() <= 0)
        <p class="text-center">
...
@endif
  <!-- Blog Post -->
        @if (is_array($specSubject->questions) || is_object($specSubject->questions))
            @foreach($specSubject->questions as $question)
            <div class="card mb-4" id="Q{{$question->id}}">
              ...
              <div class="card-footer text-muted">
                ...
                <span class="pull-right text-success pr-3">
                 Vous avez déja réagi à cette question {{$question->qcomments->where('teacher_id',$question->subject->teacher->id)->count()}}
                 fois
          ...
          <div class="card-body">
            <div class="row">
              ...
              <div class="col-sm-6 mt-2">
                    <a href="/teacher/specQuiz/{{$subject->id}}">{{$subject->name}} en {{$subject->classroom->name}}</a>
              </div>
              @endforeach
            </div>
          </div>
