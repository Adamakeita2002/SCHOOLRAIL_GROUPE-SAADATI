                ...
                <input type="hidden" value="{{ csrf_token() }}" name="_token">
                  ...
                  <div class="form-group">
                    ...
                    <select class="form-control" name="subject_id" required>
                      ...
                      <option  value="{{$subject->id }}">{{$subject->classroom->name}} / {{$subject->name}}
                      </option>
                      @endforeach
                    </select>
                  </div>
    ...
    <div class="row">
         ...
         <div class="table-responsive">
          <div class="card-body">
            ...
            ?>
              <tr>
                <td style="@if ($Ndate > $Cdate ) background-color: #ff000036; @endif " >{{$classrooms->where('id',$calendar->classroom_id)->pluck('name')->first()}}</td>

                <td style="@if ($Ndate > $Cdate ) background-color: #ff000036; @endif " >{{$subjects->where('id',$calendar->subject_id)->pluck('name')->first()}}</td>

                <td style="@if ($Ndate > $Cdate ) background-color: #ff000036; @endif " >
                  <p class="@if($calendar->epreuve_id == 1) badge badge-primary @elseif($calendar->epreuve_id == 2) badge badge-success @else badge badge-dark @endif " style="font-size: 18px" >{{$epreuves->where('id',$calendar->epreuve_id)->pluck('name')->first()}}</p>
                </td>
              ...
              <div class="modal fade" id="T{{$calendar->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <form action="{{ URL::to('/teacher/DeleteCalendar') }}" method="post" enctype="multipart/form-data" id="theform2">
