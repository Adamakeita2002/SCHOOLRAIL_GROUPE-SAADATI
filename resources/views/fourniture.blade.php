...
<table width="100%" >
  ...
  <span style="font-size: 12px"> ETATS DE PAIEMENTS <br> FOURNITURE <br> {{$academicyearP->labelYear}}</span><br>
      </h1>
...
<h1 style="text-align:center;">FOURNITURE</h1>
...
<tr style="height:10pt;" > <!-- START EN TETE -->
      ...
      <!--END HEADERF -->
<?php $T =0;  ?>
@foreach($data->versements->where('type',9) as $versement) 
...
<h4 > <span style="color:red">TOTAL:</span> {{number_format($T)}} FCFA  </h4>
        ...
        <?php $V=$data->versements->where('type',9)->count();  ?>
        @if($V<=0)
