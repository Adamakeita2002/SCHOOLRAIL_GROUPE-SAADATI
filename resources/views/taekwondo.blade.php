...
<table width="100%" >
  ...
  <span style="font-size: 12px"> ETATS DE PAIEMENTS <br> TAEKWONDO <br> {{$academicyearP->labelYear}}</span><br>
      </h1>
...
<h1 style="text-align:center;">TAEKWONDO</h1>
...
<tr style="height:10pt;" > <!-- START EN TETE -->
      ...
      <!--END HEADERF -->
<?php $T =0;  ?>
@foreach($data->versements->where('type',8) as $versement) 
...
<h4 > <span style="color:red">TOTAL:</span> {{number_format($T)}} FCFA  </h4>
        ...
        <?php $V=$data->versements->where('type',8)->count();  ?>
        @if($V<=0)
