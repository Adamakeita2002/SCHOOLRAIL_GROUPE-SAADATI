...
@if (!empty($classroom)) 
      <div class="row">
          ...
          <a href="#" class="btn btn-primary" data-toggle="modal" data-target="#MRKQ1{{$student->id}}{{$semesterP->id}}">
            <i class="icon-eye"></i>{{$semesterP->label}} </a>|
          
          <!-- Modal Edit teacher -->
            <div class="modal fade" id="MRKQ1{{$student->id}}{{$semesterP->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true"><!-- Modal Edit teacher -->
              ...
              <div class="modal-dialog modal-lg" role="document"><!--MODAL DIALOG -->
                ...
                <div class="modal-body"> <!--MODAL BODY -->
                  <form action="{{ URL::to('/kuro/admin/printBulletin2') }}" method="post" enctype="multipart/form-data" id="theformed{{$student->id}}">
                    <input type="hidden" value="{{ csrf_token() }}" name="_token">
                    <input type="hidden" name="id" value="{{$student->id}}">
                    <input type="hidden" name="academicyear_id" value="{{$classroom->academicyear->id}}">
